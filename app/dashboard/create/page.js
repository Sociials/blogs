...
export default function CreatorDashboard() {
  ...
  const fetchBlogs = async () => {
    try {
      // Calls Next.js Proxy -> Worker
      const res = await fetch("/api/creator/blogs");
      const data = await res.json();
      if (Array.isArray(data)) setBlogs(data);
      else if (data.data) setBlogs(data.data); // Handle { success: true, data: [] }
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(url, {
        method,
        body: payload, // Browser sets Content-Type to multipart/form-data automatically
      });
  ...
  // --- Render ---
  return (
    <div className="min-h-screen bg-[#F3F2EC] text-black font-sans p-6 md:p-12">
      <div className="max-w-6xl mx-auto">
        {/* Header */}
        <div className="flex justify-between items-center mb-8">
          <h1 className="unbounded-900 text-3xl font-black">
            Creator Dashboard
          </h1>
          {view === "list" && (
            <button
              onClick={handleCreateNew}
              className="bg-black text-white px-6 py-3 rounded-xl font-bold hover:bg-[#A259FF] transition-all"
            >
              + Write New
            </button>
          )}
          {view === "editor" && (
            <button
              ...
              className="border-2 border-black px-6 py-3 rounded-xl font-bold hover:bg-gray-100 transition-all"
            >
              ← Back to List
            </button>
          )}
            ...
            <div className="lg:col-span-2 space-y-6">
              <div className="bg-white p-6 rounded-2xl border-2 border-black shadow-[4px_4px_0px_#000]">
                <input
                  ...
                  className="w-full text-2xl font-black border-b-2 border-gray-200 py-2 focus:outline-none focus:border-[#A259FF]"
              ...
              <div className="bg-white p-6 rounded-2xl border-2 border-black shadow-[4px_4px_0px_#000] min-h-[500px] flex flex-col">
                ...
                {activeTab === "write" ? (
                  <textarea
                    className="flex-1 w-full resize-none outline-none font-mono text-sm"
                  ...
                  <div className="prose prose-sm max-w-none overflow-y-auto h-[400px]">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white p-6 rounded-2xl border-2 border-black shadow-[4px_4px_0px_#000] space-y-4">
                <div>
                  ...
                  <input
                    type="text"
                    className="w-full border-2 border-black rounded-lg p-2 font-bold text-sm"
                    value={formData.slug}
                    onChange={(e) =>
                ...
                <div>
                  ...
                  <div className="relative border-2 border-dashed border-black rounded-lg p-4 text-center cursor-pointer hover:bg-gray-50 mt-1">
